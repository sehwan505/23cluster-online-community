...
function App() {
	...
	useEffect(() => {
	  ...
	  if (isAuthenticated) {
		...
		const res = await fetch(`${process.env.REACT_APP_URL}/user/verify/`, {
		  method: 'POST',
		  headers: {
		...
		let res1 = await fetch(`${process.env.REACT_APP_URL}/user/current/`, {
			headers: {
		...
		let res2 = await fetch(`${process.env.REACT_APP_URL}/user/refresh/`, {
		  method: 'POST',
		  headers: {
			'Content-Type': 'application/json'
		  },
