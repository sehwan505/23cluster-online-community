...
function App() {
	...
	useEffect(() => {
	  ...
	  if (isAuthenticated) {
		...
		const res = await fetch('http://52.78.40.184:8000/user/verify/', {
		  method: 'POST',
		  headers: {
		...
		let res1 = await fetch('http://52.78.40.184:8000/user/current/', {
			headers: {
		...
		let res2 = await fetch('http://52.78.40.184:8000/user/refresh/', {
		  method: 'POST',
		  headers: {
			'Content-Type': 'application/json'
		  },
