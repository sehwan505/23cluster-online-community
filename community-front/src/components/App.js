...
function App() {
	...
	useEffect(() => {
	  ...
	  if (isAuthenticated) {
		...
		const res = await fetch('https://23cluster.com/user/verify/', {
		  method: 'POST',
		  headers: {
		...
		let res1 = await fetch('https://23cluster.com/user/current/', {
			headers: {
		...
		let res2 = await fetch('https://23cluster.com/user/refresh/', {
		  method: 'POST',
		  headers: {
			'Content-Type': 'application/json'
		  },
