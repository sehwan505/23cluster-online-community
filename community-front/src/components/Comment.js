...
const Comment = ({ comment, isOwner, user, post_id, isAuthenticated }) => {
  ...
  const onSubmit = async (event) => {
    ...
    await axios.post(`https://13.124.51.99:80/api/post/add_comment/${post_id}/`, {
        post_id:post_id,
  ...
  const onLikeClick = async () => {
	...
	await axios.post(`https://13.124.51.99:80/api/post/like_comment/${comment.comment_id}/`, {
  ...
  const onUnlikeClick = async () => {
	...
	await axios.post(`https://13.124.51.99:80/api/post/unlike_comment/${comment.comment_id}/`, {
		category: user.category
	}, config).then((response) => {
	// 응답 처리
	})
  ...
  const onDeleteClick = async () => {
    ...
    if (ok) {
        ...
        await axios.post(`https://13.124.51.99:80/api/post/delete_comment/${comment.comment_id}/`, {
        }, config).then((response) => {
        // 응답 처리
        })
        .catch((error) => {
