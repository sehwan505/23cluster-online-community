...
function Profile({user, handleLogout, isAuthenticated}){
  ...
  async function fetchComment(){
	try {
		...
		const res = await axios.post(`https://23cluster.com:80/api/post/profile/`,{} ,config);
  ...
  const onsubmit = async (event) =>{
    ...
    if(user.username.localeCompare(newDisplayName)){
		await axios.put(`https://23cluster.com:80/user/profile/${user.user_pk}/update/`, {
  ...
  const onResignClick = async () =>{
	...
	if (ok){
		let res = await fetch('https://23cluster.com:80/user/resign/', {
			headers: {
  ...
  const onRefreshClick = async () => {
	...
	await axios.post('https://23cluster.com:80/user/refresh_category/', {
    }, config)
  } 
//  {/*<Link to="/">홈</Link> <br/>
