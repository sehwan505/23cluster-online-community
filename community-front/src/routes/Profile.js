...
function Profile({user, handleLogout, isAuthenticated}){
  ...
  const onsubmit = async (event) =>{
    ...
    if(user.username.localeCompare(newDisplayName)){
		await axios.put(`http://52.78.40.184:80/user/profile/${user.user_pk}/update/`, {
  ...
  const onResignClick = async () =>{
	...
	if (ok){
		let res = await fetch('http://52.78.40.184:80/user/resign/', {
			headers: {
  ...
  const onRefreshClick = async () => {
	...
	await axios.post('http://52.78.40.184:80/user/refresh_category/', {
    }, config)
  } 
//  {/*<Link to="/">홈</Link> <br/>
