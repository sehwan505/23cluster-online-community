...
function Detail({user, post_id, handleLogout ,isAuthenticated}){
  ...
  async function fetchComment(){
	try {
		const res2 = await fetch(`http://52.78.40.184:8000/api/post/detail_comment/${id}/?page=${commentPageNum}`);
  ...
  useEffect(async() => {
		try {
			window.scrollTo(0,0);
			const res = await fetch(`http://52.78.40.184:8000/api/post/detail/${id}/`)
...
useEffect(async() => {
	try{
		...
		const res1 = await fetch(`http://52.78.40.184:8000/api/post/section/${posts.section}/?page=${pageNum}`);
...
const onSubmit = async (event) => {
    ...
    await axios.post(`http://52.78.40.184:8000/api/post/add_comment/${id}/`, {
        post_id:id,
  ...
  const onDeleteClick = async () => {
    ...
    if (ok) {
        ...
        await axios.post(`http://52.78.40.184:8000/api/post/delete_post/${id}/`, {
        }, config).then((response) => {
		  alert('삭제 완료');
		  history.push('/');
        })
  ...
  const sidebarLike = async () => {
	...
	await axios.post(`http://52.78.40.184:8000/api/post/like_post/${id}/`, {
	}, config).then((response) => {
	// 응답 처리
	})
	.catch((error) => {
  ...
  const sidebarUnlike = async () => {
	...
	await axios.post(`http://52.78.40.184:8000/api/post/unlike_post/${id}/`, {
	}, config).then((response) => {
	// 응답 처리
	})
	.catch((error) => {
